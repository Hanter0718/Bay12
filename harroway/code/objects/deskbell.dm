...
/*
/obj/item/device/pda/proc/new_message(var/sender, var/sender_job, var/message)
...
/obj/item/device/deskbell
	...
	icon = 'harroway/icons/obj/deskbell.dmi'
	...
	hitsound = 'harroway/sound/machines/ding2.ogg'
	...
	//var/cartridge_name = "generic cartridge"
		...
		if(do_after(user, src, 30))
			if(src)
				...
				user.visible_message(
					...
					"<span class='notice'>You [anchored ? "wrench" : "unwrench"] \the [src] [anchored ? "in place" : "from its fixture"].</span>",
...
/obj/item/device/deskbell/proc/ring()
	if(world.time - last_ring_time >= ring_delay)
		...
		playsound(src, 'harroway/sound/machines/ding2.ogg', 50, 1)
		return 1
	return 0

/obj/item/device/deskbell/MouseDrop(mob/user as mob)
	if(user == usr && !usr.incapacitated() && (usr.contents.Find(src) || in_range(src, usr)))
		if(istype(user, /mob/living/carbon/human)/* || istype(user, /mob/living/monkey)*/)
			if(anchored)
...
/obj/item/device/deskbell/signaler/ring()
	if(..())
		...
		switch (frequency)
			if(1200)
				jobs_list = list("Quartermaster", "Cargo Technician", "Shaft Miner")
			if(1202)
				jobs_list = list("Head of Personnel")
			if(1204)
				jobs_list = list("Chief Medical Officer", "Medical Doctor", "Geneticist", "Virologist")
			if(1206)
				jobs_list = list("Head of Security", "Warden", "Detective", "Security Officer")
			if(1208)
				jobs_list = list("Research Director", "Scientist")
			else
				jobs_list = list()
		...
		for(var/obj/item/device/pda/ring_pda in PDAs)
			...
			//var/obj/item/weapon/cartridge/Cart = ring_pda.cartridge

			if(ring_pda.ownjob in jobs_list)

				var/turf/T = get_turf(ring_pda)
				playsound(T, 'harroway/sound/machines/notify.ogg', 50, 1)
				T.visible_message("*[src.name]*")
...
/obj/item/device/deskbell_assembly/attackby(obj/item/W, mob/user)
	if(istype(W, /obj/item/weapon/pen))
		var/t = copytext(input(user, "Enter new desk bell name", src.name, final_name),1,MAX_NAME_LEN)
		if (!t)
			return
		...
		switch(build_step)
			if(0)
				if(iswrench(W))
					...
					var/obj/item/stack/material/steel/M = PoolOrNew(/obj/item/stack/material/steel, get_turf(src))
					M.amount = 2
					qdel(src)
					return
...
var/global/list/deskbell_default_frequencies = list(1200, 1202, 1204, 1206, 1208)
var/global/deskbell_freq_cargo = 1200//call(/obj/item/device/deskbell/signaler/proc/get_new_bellfreq)()
var/global/deskbell_freq_hop = 1202//call(/obj/item/device/deskbell/signaler/proc/get_new_bellfreq)()
var/global/deskbell_freq_medbay = 1204//call(/obj/item/device/deskbell/signaler/proc/get_new_bellfreq)()
var/global/deskbell_freq_brig = 1206//call(/obj/item/device/deskbell/signaler/proc/get_new_bellfreq)()
var/global/deskbell_freq_rnd = 1208//call(/obj/item/device/deskbell/signaler/proc/get_new_bellfreq)()
